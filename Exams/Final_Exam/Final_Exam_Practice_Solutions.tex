...
\noindent \textbf{How to use this document.} For each practice question, the question is restated and then followed by a worked solution that emphasizes the \emph{reasoning} behind the answer. Where more than one defensible response exists, both are shown so you can see what the grader is willing to accept. Numerical figure-based answers refer to the practice-questions PDF and assume \texttt{random\_state=1706}.
...
\noindent \textbf{Note.} These solutions are model responses at the depth expected on the final exam; substantively correct reasoning phrased differently receives full credit. Numerical answers for figure-based questions assume \texttt{random\_state=1706} and are based on the figures generated by \texttt{generate\_figures\_final\_exam{1,2,3}.py}.
